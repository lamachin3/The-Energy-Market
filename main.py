...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(usage='main.py -n NB_HOUSES -d NB_DAYS -p NB_POLICY_1 NB_POLICY_2 NB_POLICY_3')
    ...
    market = Market(temperature, barrier, stop)
    market.start()
    ...
    if not args.policies:
        for i in range(args.houses):
            homes.append(Home(random.randint(1,3), temperature, barrier, stop))
    else:
        policy = 1
        for i in args.policies:
            i = int(i)
            for j in range(i):
                homes.append(Home(policy, temperature, barrier, stop))
            policy += 1
